...
class TransformerVAE(tf.keras.Model):
    def __init__(self, input_dim, latent_dim=64, embed_dim=128, num_heads=4, ff_dim=128,
                 num_encoder_layers=3, num_decoder_layers=3, dropout_rate=DROPOUT, beta=0.01):
        super().__init__()
        ...
        # Encoder MultiHeadAttention + FFN Layer 반복
        ...
        for _ in range(num_encoder_layers):
            self.encoder_attn_layers.append(layers.MultiHeadAttention(num_heads=num_heads, key_dim=embed_dim))
            self.encoder_norm1_layers.append(layers.LayerNormalization())
            self.encoder_dropout1_layers.append(layers.Dropout(dropout_rate))
            self.encoder_ffn_layers.append(tf.keras.Sequential([
                layers.Dense(ff_dim, activation='relu'),
                layers.Dense(embed_dim)
            ]))
            self.encoder_norm2_layers.append(layers.LayerNormalization())
            self.encoder_dropout2_layers.append(layers.Dropout(dropout_rate))
        ...
        self.decoder_attn_layers = []
        self.decoder_ffn_layers = []
        self.decoder_norm1_layers = []
        self.decoder_norm2_layers = []
        self.decoder_dropout1_layers = []
        self.decoder_dropout2_layers = []
        for _ in range(num_decoder_layers):
            self.decoder_attn_layers.append(layers.MultiHeadAttention(num_heads=num_heads, key_dim=embed_dim))
            self.decoder_norm1_layers.append(layers.LayerNormalization())
            self.decoder_dropout1_layers.append(layers.Dropout(dropout_rate))
            self.decoder_ffn_layers.append(tf.keras.Sequential([
                layers.Dense(ff_dim, activation='relu'),
                layers.Dense(embed_dim)
            ]))
            self.decoder_norm2_layers.append(layers.LayerNormalization())
            self.decoder_dropout2_layers.append(layers.Dropout(dropout_rate))
    ...
    def call(self, inputs):
        ...
        x = self.expand(x)
        for attn, norm1, drop1, ffn, norm2, drop2 in zip(
            self.encoder_attn_layers,
            self.encoder_norm1_layers,
            self.encoder_dropout1_layers,
            self.encoder_ffn_layers,
            ...
            attn_out = attn(x, x)
            x = drop1(norm1(x + attn_out))
            ffn_out = ffn(x)
            x = drop2(norm2(x + ffn_out))
        ...
        for attn, norm1, drop1, ffn, norm2, drop2 in zip(
            ...
            x_dec = drop1(norm1(x_dec + attn_out))
            ffn_out = ffn(x_dec)
            x_dec = drop2(norm2(x_dec + ffn_out))
