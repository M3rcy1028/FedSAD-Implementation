...
from utils import *
from model_taae_rne0p_valid import SaveEvaluationRNEP, TransformerAAE, FLClient  # FLClient class can be used if imported

os.makedirs("./rnep_frame_valid", exist_ok=True)
WEIGHT_PATH = "./rnep_frame_valid/rnep_frame_aae_transformer_weights.h5"
MATRIX_PATH = "./rnep_frame_valid/rnep_frame_cm.png"
RESULT_PATH = "./rnep_frame_valid/rnep_frame_server.txt"
ROC_PATH = "./rnep_frame_valid/rnep_frame_roc.png"
CSV_PATH = "./rnep_frarnep_frame_validme_revised/rnep_frame_history"
PNG_PATH = "./rnep_frame_valid/rnep_frame_history.png"

os.environ["CUDA_VISIBLE_DEVICES"] = "-1" 

def main():
    args = get_args()
    X_train_scaled, X_test_full, y_test_full = get_datasets_insdn()
    client_data = np.array_split(X_train_scaled, args.client_nums)

    ####################################################
    # 2. stratify 옵션을 사용하여 test_full을 valid와 test로 분리합니다.
    ####################################################
    X_valid, X_test, y_valid, y_test = train_test_split(
        X_test_full, 
        y_test_full, 
        test_size=0.9,  # test_size=0.9는 valid 셋을 10%로 만듭니다.
        random_state=args.random_seed,
        stratify=y_test_full  # 👈 핵심: 0과 1의 비율을 동일하게 유지
    )
    ...
    eval_server_args = {
        ...
        # [수정 2] X_test_scaled -> X_valid
        "X_test_scaled": X_valid, 
        # [수정 3] y_test -> y_valid
        "y_test": y_valid,
    ...
    def client_fn(cid: str):
        ...
        return FLClient(
            cid_int,                # 1. cid
            client_model,           # 2. model
            client_data[cid_int],   # 3. X_train
            X_valid,                # 4. X_valid (👈 X_test_full 제거)
            y_valid,                # 5. y_valid
            epochs=args.client_epochs # 6. epochs (키워드)
        )
    ...
    eval_server(
        ...
        X_train_scaled,
        X_test_full,
        # [수정 6-1] X_test_scaled -> X_test
        X_test,
        # [수정 6-2] y_test -> y_test
        y_test,
        result_path=RESULT_PATH,
        matrix_path=MATRIX_PATH,
        roc_path=ROC_PATH
