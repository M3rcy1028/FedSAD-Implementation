...
def run_anomaly_detection():
    ...
    try:
        df_normal_all = pd.read_csv('CIC_normal.csv')
    except FileNotFoundError:
        print("❌ CIC_normal.csv 파일을 찾을 수 없습니다.")
        return
    
    # ---- 데이터셋 나누기 ----
    df_normal_train = df_normal_all.iloc[:NUM_SAMPLES].copy()
    # 평가 데이터: 나머지
    df_normal_eval = df_normal_all.iloc[NUM_SAMPLES:].copy()
    ...
    # --- 2. 평가용 데이터 준비 (정상 뒷부분 + 이상) ---
    print("\n--- 2. 평가 데이터 준비 ---")
    try:
        df_anomaly_eval = pd.read_csv('CIC_anomaly.csv')
    except FileNotFoundError:
        print("❌ CIC_anomaly.csv 파일을 찾을 수 없습니다.")
        return

    df_anomaly_eval['Timestamp'] = pd.to_datetime(df_anomaly_eval['Timestamp'])
    df_anomaly_eval.set_index('Timestamp', inplace=True)

    y_true, y_pred = [], []

    # --- 3. 정상 데이터 평가 ---
    for period, df_window in df_normal_eval.groupby(pd.Grouper(freq=TIME_WINDOW)):
        graph = create_graph_from_window(df_window)
        if not graph:
            continue
        ...
        y_true.append(0)  # 실제: Normal

    # --- 4. 이상 데이터 평가 ---
    for period, df_window in df_anomaly_eval.groupby(pd.Grouper(freq=TIME_WINDOW)):
        graph = create_graph_from_window(df_window)
        if not graph:
            continue
        features = extract_graph_features(graph)
        ...
        for feature_name in features:
            ...
            if not (lower <= features[feature_name] <= upper):
                ...
                break
        y_pred.append(is_anomaly)
        y_true.append(1)  # 실제: Anomaly

    # --- 5. 성능 평가 ---
    print(f"\n총 {sum(y_pred)}개의 시간대에서 이상 탐지됨.")
