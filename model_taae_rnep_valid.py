...
class FLClient(fl.client.NumPyClient):
    # [수정 1] X_test, y_test -> X_valid, y_valid
    def __init__(self, cid, model, X_train, X_valid, y_valid, epochs=50):
        self.cid = cid
        self.model = model
        self.X_train = X_train
        
        # [수정 2] self.X_valid, self.y_valid로 저장
        self.X_valid = X_valid
        self.y_valid = y_valid
        
        self.epochs = epochs
        # 'model_aae_rnep'에 따라 입력 차원 수정 (이전 코드와 동일하게)
        self.model(tf.zeros((1, X_train.shape[1])), prior_labels=tf.zeros((1,1)))
    ...
    def fit(self, parameters, config):
        ...
        self.model.fit(self.X_train, self.X_train,
                       ...
                       verbose=2,
                       validation_split=0.2) # 👈 이 부분
        ent = compute_client_entropy(self.X_train)
        return self.model.get_weights(), len(self.X_train), {"entropy": float(ent)}

    def evaluate(self, parameters, config):
        # 로컬 결과 얻기 (threshold는 train 데이터로 계산 - 이 부분은 올바름)
        self.model.set_weights(parameters)
        ...
        # [수정 3] 'valid' 데이터셋으로 성능 평가하기
        pred_valid = self.model.predict(self.X_valid, verbose=0) # 👈 X_test -> X_valid
        recon_errors_valid = np.mean(np.square(self.X_valid - pred_valid), axis=1) # 👈 X_test -> X_valid
        y_pred = (recon_errors_valid > threshold).astype(int)

        # [수정 4] 'valid' 레이블과 비교
        acc = np.mean(y_pred == self.y_valid) # 👈 y_test -> y_valid
        report = classification_report(
            self.y_valid, y_pred, # 👈 y_test -> y_valid
            target_names=["Normal", "Anomaly"],
            zero_division=0
        )
        
        # (콘솔/파일 출력 코드는 그대로 둬도 됩니다)
        print(f"\n--- Client {self.cid} Evaluation (using Validation Set) ---")
        # ... (이하 생략) ...

        # [수정 5] valid 셋의 길이 반환
        return float(1 - acc), len(self.X_valid), {} # 👈 len(self.X_test) -> len(self.X_valid)